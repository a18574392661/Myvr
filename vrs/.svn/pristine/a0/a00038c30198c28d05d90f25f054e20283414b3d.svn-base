...
public class RSAUtil {
	...
	//private static final String PKSC8_PUBLIC = ""; 
	 ...
	 * PublicKey publicKey = null; InputStream inputStream = null; try {
	 ...
	 * } catch (Exception e) {
	 * 
	 * e.printStackTrace();//EAD PUBLIC KEY ERROR System.out.println("加载公钥出错!"); }
	 * finally { if (inputStream != null){ try { inputStream.close(); }catch
	 * (Exception e){ System.out.println("加载公钥,关闭流时出错!"); } } } return publicKey; }
	 */
	public static PublicKey getPublicKey(String keyAlgorithm) throws Exception {
		try {
			/*
			 * System.out.println("b1........."); BufferedReader br = new BufferedReader(new
			 * InputStreamReader(inputStream)); System.out.println("b2.........");
			 * StringBuilder sb = new StringBuilder(); String readLine = null;
			 * System.out.println("b3........."); while ((readLine = br.readLine()) != null)
			 * { if (readLine.charAt(0) == '-') { continue; } else { sb.append(readLine);
			 * sb.append('\r'); } } System.out.println("b4.........");
			 */
			//ClassPathResource classPathResource = new ClassPathResource("publickey.pem");
			//InputStream is = classPathResource.getInputStream();
			
			//BufferedReader br = new BufferedReader(new FileReader(classPathResource.getFile())); 
			BufferedReader br = new BufferedReader(new FileReader(new File("/opt/project/key/public_key_pkcs8.pem"))); 
			//BufferedReader br = new BufferedReader(new FileReader(new File("C:\\Users\\shilic\\Desktop\\1536182901_20190803_cert\\public_key_pkcs8.pem"))); 
			...
			/*
			 * //读取公钥 X509EncodedKeySpec pubX509 = new
			 * X509EncodedKeySpec(decodeBase64(sb.toString()));
			 */
			 ...
			 * finally { try { if (inputStream != null) { inputStream.close(); } } catch
			 * (IOException e) { inputStream = null; throw new
			 * Exception("INPUT STREAM CLOSE ERROR:", e); } }
			 */
	}
