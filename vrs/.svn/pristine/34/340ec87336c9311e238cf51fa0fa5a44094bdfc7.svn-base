...
//var _host = "http://192.168.0.102:8083";//"http://192.168.48.82:8083";//
var _host = "";//"http://192.168.0.108:8083";

//var _host = "http://hydjxapp.com";
...
var Request = function (params, callback){
	...
	if( !sessionId ){
		...
		mui.confirm('为了更好的阅读体验,请下载鸿运达驾校APP后再操作!', '温馨提示', ['取消','立即下载'], function(e) {
			if(e.index == 1) {
				setTimeout(function() {
					location.href = "/api/down";
				}, 200);
			}
		}, 'div');
...
function getImage(callback){
	...
	cmr.captureImage(function(path){
		...
		plus.gallery.save(path, function(){
			console.log('保存成功');
			callback.call(this, path );//上传成功之后回调
			//TODO 上传到服务器或保存到本地的某个位置
		}, function(e){
