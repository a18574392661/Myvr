   ...
   model.addAttribute("dx", titleClsService.queryTypeId(1));
	...
	String edit(@PathVariable("id") Integer id,Model model){
		...
		if (checks!=null&&checks.length>0){
			String pid=checks[0];
			//懒得优化了 出效果就行了
			ImgClsDO imgClsDO=imgClsDao.get(Long.parseLong(pid));
			ImgClsDO imgClsDO2=imgClsDao.get(Long.parseLong(imgClsDO.getPid()+""));
			model.addAttribute("pid",imgClsDO2.getPid());//一级父id
		}
	...
	//编辑异步选中
	@ResponseBody
	@RequestMapping("/to_editClsChecked")
	public List<ImgClsDO>  to_editClsChecked(String id,String psid){
  		Map<String,Object> ms=new HashMap<>();

		String[] checks=imgClsService.queryProCeebnCls(Integer.parseInt(id));
		//String pid=checks[0];
		//懒得优化了 出效果就行了
		/*ImgClsDO imgClsDO=imgClsDao.get(Long.parseLong(pid));
		ImgClsDO imgClsDO2=imgClsDao.get(Long.parseLong(imgClsDO.getPid()+""));*/
		//ms.put("pid",imgClsDO2.getPid());//一级父id

		List<ImgClsDO> list=imgClsService.imgClsPars(psid);
		for (int i = 0; i <list.size() ; i++) {
			List<ImgClsDO> chis=list.get(i).getChilds();
			if (chis!=null&&chis.size()>0){
				...
				for (int j = 0; j <checks.length ; j++) {
					...
					for (int k = 0; k <chis.size() ; k++) {
						...
						if (Long.parseLong(checks[j])==chis.get(k).getId()){
							chis.get(k).setChecked(true);
							break;
						}
