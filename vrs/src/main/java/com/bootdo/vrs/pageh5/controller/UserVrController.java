...
public class UserVrController  extends BaseController {
    ...
    @GetMapping("/login")
    public String login(HttpServletRequest request,@RequestParam(defaultValue = "/vrs/houses/index") String url,String twoIndex,ModelMap map){
        ...
        try {
            ...
            if (userdoSucess != null) {
                ...
                if (StringUtils.isNotBlank(token)){
                    ...
                    //保存到后台登录
                    if ("vr".equals(userdoSucess.getIdfind())) {
    ...
    @ResponseBody
    @RequestMapping("/isUsernotVip")
    @LoginRequired(loginSuccess = false)
    public  boolean isUsernotVip(HttpServletRequest request){
        String uid=request.getAttribute("uid")+"";
        if (StringUtils.isBlank(uid)||"null".equals(uid)){
            return true;
        }
        UserDO u= userDao.get(Long.parseLong(uid));
        System.out.println(u.getIsVip());
        if (u.getIsVip()==null||u.getIsVip()==0){
            return  true;
        }
